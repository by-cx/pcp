...
class PHPApp(AppObject):
    ...
    def update(self):
        ...
        self.script.add_file("/var/www/%(user)s/php-wrap" % parms, owner=self.get_user(), self.gen_php_wrap())
        self.script.add_file("/var/www/%(user)s/php.ini" % parms, owner=self.get_user(), self.gen_php_ini())
