...
class formReg(forms.Form):
	company		= forms.CharField(label=_(u"Název firmy"),max_length=250,required=False)
...
class formReg2(forms.Form):
	...
	def clean_username(self):
		if len(user.objects.filter(username=self.cleaned_data["username"])) != 0: raise forms.ValidationError(_(u"Uživatelské jméno je již obsazeno"))
...
class form_reg_payment(forms.Form):
	pay_method		= forms.ChoiceField(label=_(u""), required=True, choices=(("per_web","Za každou aplikaci (60Kč/app/měsíc)"), ("fee","Paušální poplatek (200 Kč/196 MB RAM)")))
