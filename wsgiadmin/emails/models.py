...
class formEmail(ModelForm):
    ...
    def clean_login(self):
        d = Domain.objects.filter(name=self.data["xdomain"])[0]
        if not email.objects.filter(remove=False, domain=d, login=self.cleaned_data["login"]).count():
...
class formRedirect(ModelForm):
    ...
    def clean_login(self):
        d = Domain.objects.get(name=self.data["domain"])
        if not email.objects.filter(domain=d, login=self.cleaned_data["alias"]).count():
