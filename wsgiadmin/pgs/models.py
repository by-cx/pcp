...
class form_db(forms.Form):
    database = forms.CharField(label=_(u"Název databáze"), help_text=u"Název databáze (i vytvořené uživatelské jméno) bude doplněn o prefix ve tvaru <i>prefix_názevdatabáze</i>, kde prefix jsou první tři znaky vašeho uživatelského jména.")
    password = forms.CharField(label=_(u"Heslo k databázi"), widget=forms.PasswordInput)

    def clean_database(self):
        if len(self.cleaned_data["database"]) > 16:
            raise forms.ValidationError(_(u"Název databáze může mít maximálně 16 znaků"))
