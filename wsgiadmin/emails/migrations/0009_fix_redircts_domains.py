...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for domain in orm['domains.domain'].objects.filter(mail=True):
            if domain.emailredirect_set.count() and not domain in save:
                print "Domain %s will be migrated" % domain.name
                save.append(domain)
        for domain in save:
            if not orm["emails.domain"].objects.filter(name=domain.name).count():
                ...
                new_domain = orm["emails.domain"].objects.get(name=domain.name)
            for redirect in domain.emailredirect_set.all():
                print "\t%s@%s migrated" % (redirect.alias, new_domain.name)
                redirect.new_domain = new_domain
                redirect.save()
