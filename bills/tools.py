...
def bill_services(now=datetime.date.today()):
	...
	for s in service.objects.all():
		if s.one_day_price() == 0:
			continue
		if kget("bill:service:%d:%d:%d:%d"%(s.id,day,month,year)) != "processed":
			...
			logging.info("Vygenerována platba za službu %s pro uživatele %s v hodnotě %.2f"%(s.info,s.user.username,s.one_day_price()*s.user.parms.dc()))
	
			kset("bill:service:%d:%d:%d:%d"%(s.id,day,month,year),"processed",20160)
...
def bill_hosting(now=datetime.date.today()):
	...
	for u in user.objects.all():
		#Fee process
		if u.parms.fee:
			if day == u.date_joined.day and kget("bill:hosting:fee:%d:%d:%d:%d"%(u.id, day, month, year)) != "processed":
				...
				kset("bill:hosting:fee:%d:%d:%d:%d"%(u.id, day, month, year), "processed", 20160)
	...
	for s in site.objects.all():
		...
		logging.info("Vygenerována platba za hosting doménu %s pro uživatele %s v hodnotě %.2f"%(s.serverName,s.owner.username,s.pay()))
		...
		if data:
			...
			for line in data:
				...
				if data[line][1].date() == data[line][2].date():
					itm.name			= "%s - od %s do %s"%(line,data[line][1].strftime("%d.%m.%Y"),data[line][2].strftime("%d.%m.%Y"))
				else:
					itm.name			= "%s - od %s do %s"%(line,data[line][1].strftime("%d.%m.%Y"),data[line][2].strftime("%d.%m.%Y"))
				itm.count			= 1
			...
			if process:
				logging.info("Vygenerována faktura %d pro uživatele %s v hodnotě %s"%(inv.payment_id,u.username,inv.total()))
			else:
				print "Faktura pro uživatele %s v hodnotě %s kč"%(u.username, total1)
